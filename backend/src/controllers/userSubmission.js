...
const prepareExecutableCode = (code, language, tag = 'array') => {
    ...
    const hasTargetParam = code.includes('target') || code.includes('amount') || code.includes('Target');
    const isSingleIntParam = (code.includes('int n') || code.includes('n: int') || code.includes('solve(n)') || code.includes('solve(n:')) && !code.includes('int*') && !code.includes('vector') && !code.includes('int[]');
    ...
    if (lang === 'c++' || lang === 'cpp') {
        ...
        if (!code.includes('int main')) {
            ...
            if (problemTag === 'linkedlist') {
                const solveCall = isTwoLists ? 'sol.solve(dummy1.next, dummy2.next)' : 'sol.solve(dummy1.next)';
    ...
    if (lang === 'c') {
        ...
        if (problemTag === 'linkedlist' && !code.includes('struct ListNode')) {
            headers += `struct ListNode { int val; struct ListNode* next; };\n\n`;
        }
        ...
        if (!code.includes('int main')) {
            ...
            if (problemTag === 'linkedlist') {
                const solveCall = isTwoLists ? 'solve(dummy1.next, dummy2.next)' : 'solve(dummy1.next)';
